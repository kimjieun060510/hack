...
| 바꾸고 싶은 것 | 파일 |
| --- | --- |
| 학교·학과 공지 HTML 파서, 아이캠퍼스/에타 예시 JSON | `lib/feeds.dart`, `assets/feeds/` |
...
- 학교·학과 공지는 공개 게시판 HTML을 읽어 추천 탭에 넣어요. 아이캠퍼스·에브리타임은 공식 API가 없어서 로그인 정보를 받아 긁지 않고, `assets/feeds/` 의 예시 JSON을 씁니다. 자세한 설명은 저장소 루트 `README.md` 를 보세요.
